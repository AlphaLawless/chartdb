...
export const useUpdateTableField = (
  ...
  const updateField = useMemo(
    () =>
      customUpdateField
        ? (_tableId: string, _fieldId: string, attrs: Partial<DBField>) =>
  ...
  const dataFieldOptions = useMemo(() => {
    ...
    const customTypeOptions: SelectBoxOption[] = customTypes.map((type) => ({
      label: type.name,
      value: type.name,
      description: type.kind === 'enum' ? `${type.values?.join(' | ')}` : '',
      group: 'Custom Types',
    }));
    ...
    (value, regexMatches) => {
      ...
      if (regexMatches?.length) {
        if (dataType?.fieldAttributes?.hasCharMaxLength) {
          ...
          scale = regexMatches[2] ? parseInt(regexMatches[2]) : undefined;
