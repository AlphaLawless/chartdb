...
export const exportBaseSQL = ({
  ...
  if (diagram.customTypes && diagram.customTypes.length > 0) {
    diagram.customTypes.forEach((customType) => {
      ...
      if (
        ...
        if (targetDatabaseType === DatabaseType.POSTGRESQL && !isDBMLFlow) {
      ...
      diagram.customTypes.some(
        (ct) =>
          ...
          (ct.kind === 'composite' &&
            ct.fields &&
            ct.fields.length > 0 &&
            (targetDatabaseType === DatabaseType.POSTGRESQL || isDBMLFlow))
      )
    ) {
      sqlScript += '\n';
    }
  ...
  nonViewTables.forEach((table) => {
    ...
    table.fields.forEach((field, index) => {
      ...
      if (typeName.toLowerCase() === 'char' && !field.characterMaximumLength) {
      ...
      if (field.default && !field.increment) {
        ...
        if (
          ...
          if (fieldDefault.includes('::')) {
            ...
            if (
              (fieldDefault.startsWith('(') && !fieldDefault.endsWith(')')) ||
  ...
  relationships?.forEach((relationship) => {
    ...
    if (sourceTable && targetTable && sourceTableField && targetTableField) {
      ...
      const quotedFkFieldName = getQuotedFieldName(fkField.name, isDBMLFlow);
      const quotedRefFieldName = getQuotedFieldName(refField.name, isDBMLFlow);
...
export const exportSQL = async (
  ...
  const baseUrl = window?.env?.OPENAI_API_ENDPOINT ?? OPENAI_API_ENDPOINT;
  const modelName =
    window?.env?.LLM_MODEL_NAME ?? LLM_MODEL_NAME ?? 'gpt-4o-mini-2024-07-18';
