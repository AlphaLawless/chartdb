      ...
      .map((table: DBTable) => {
        ...
        return `${
          ...
          .map((field: DBField) => {
            ...
            const typeName = mapPostgresType(field.type.name, field.name);
            ...
            const unique = !field.primaryKey && field.unique ? ' UNIQUE' : '';

            // Handle default value using PostgreSQL specific parser
            const defaultValue =
              field.default && !field.default.toLowerCase().includes('identity')
          ...
          primaryKeyFields.length > 0
            ? `,\n    ${(() => {
                ...
                return pkIndex?.name ? `CONSTRAINT "${pkIndex.name}" ` : '';
              ...
              .map((index) => {
                ...
                if (
                  ...
                  primaryKeyFields.every((pk) =>
                    indexFields.some((field) => field && field.id === pk.id)
                  )
                ) {
                  return '';
                }
  ...
  if (relationships.length > 0) {
    ...
    const foreignKeys = relationships
      .map((r: DBRelationship) => {
        ...
        if (r.sourceCardinality === 'one' && r.targetCardinality === 'many') {
