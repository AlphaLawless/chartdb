...
FOR EACH ROW EXECUTE FUNCTION update_timestamp();
    ...
    expect(
      result.warnings.some((w) => w.message.includes('Function definitions'))
    ).toBe(true);
    expect(
      result.warnings.some((w) => w.message.includes('Trigger definitions'))
    ).toBe(true);
  });
...
FROM towers t;
    ...
    expect(
      result.errors.filter((e) => e.message.includes('Invalid cast operator'))
        .length
    ).toBeGreaterThan(0);
    expect(result.fixedSQL).toBeDefined();
