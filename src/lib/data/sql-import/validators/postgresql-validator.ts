...
export function validatePostgreSQLDialect(sql: string): ValidationResult {
  ...
  if (!sqlKeywords.test(sql)) {
    errors.push({
      ...
      type: 'syntax',
      suggestion: 'Ensure your SQL contains valid statements like CREATE TABLE',
    });
    return {
      isValid: false,
      errors,
    ...
    if (decimalSplitRegex.test(line) && index < lines.length - 1) {
      ...
      if (/^\d+\s*\)/.test(nextLine)) {
        errors.push({
          ...
          type: 'syntax',
          suggestion: 'Keep DECIMAL(precision, scale) on a single line',
        });
      }
    }
  });
  ...
  if (
    errors.some((e) => e.message.includes('DECIMAL type declaration is split'))
  ) {
