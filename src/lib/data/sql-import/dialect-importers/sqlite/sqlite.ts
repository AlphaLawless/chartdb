  ...
  try {
    ...
    ast.forEach((stmt: SQLASTNode) => {
      ...
      // Process CREATE INDEX statements
        ...
        processCreateIndexStatement(stmt as CreateIndexStatement, tables);
      }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Process column definitions inside CREATE TABLE
      ...
      if (match) {
        ...
        const isPrimaryKey = restOfLine.toUpperCase().includes('PRIMARY KEY');
        ...
        if (rawType === 'INTEGER' || rawType === 'INT') {
          ...
          ['REAL', 'FLOAT', 'DOUBLE', 'NUMERIC', 'DECIMAL'].includes(rawType)
        ) {
          columnType = 'REAL';
        } else if (rawType === 'BLOB' || rawType === 'BINARY') {
          columnType = 'BLOB';
        } else if (['TIMESTAMP', 'DATETIME', 'DATE'].includes(rawType)) {
    ...
    createTableStmt.create_definitions.forEach((def) => {
      if ('column' in def) {
        ...
        if (columnDef.dataType) {
          ...
          typeName = columnDef.dataType.dataType?.toUpperCase() || 'TEXT';
        ...
        columns.push({
          ...
          default: defaultValue,
          increment: isPrimaryKey && typeName.toLowerCase() === 'integer',
          typeArgs:
...
function addPlaceholderTablesForFKReferences(
  ...
  missingTableNames.forEach((tableName) => {
    ...
    const columns: SQLColumn[] = Array.from(targetColumns).map((colName) => ({
      name: colName,
      type: 'unknown',
