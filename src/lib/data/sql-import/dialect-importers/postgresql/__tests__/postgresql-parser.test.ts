...
2),
  ...
  describe('Foreign Key Relationships', () => {
    ...
    it('should parse table-level foreign key constraints', async () => {
      ...
      expect(result.relationships).toHaveLength(1);
      expect(result.relationships[0].sourceTable).toBe('enchantment_items');
      expect(result.relationships[0].targetTable).toBe('enchantment_orders');
    });
  ...
  describe('Constraints', () => {
    it('should parse unique constraints', async () => {
      ...
      expect(result.tables).toHaveLength(1);
      const columns = result.tables[0].columns;
      expect(columns.find((c) => c.name === 'magic_email')?.unique).toBe(true);
    });
