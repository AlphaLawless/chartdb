...
CREATE TABLE dragon_quests (
    ...
    if (missingEnums.length > 0) {
      ...
      missingEnums.forEach((enumName) => {
        ...
        if (regex.test(sql)) {
          console.log(`  ${enumName} exists in SQL but wasn't parsed`);
    ...
    console.log('\nCustom types in diagram:', diagram.customTypes?.length || 0);
    ...
    if (spellbooksTable) {
      ...
      if (frequencyField) {
        console.log(`  cast_frequency field type: ${frequencyField.type.name}`);
      }
      ...
      if (schoolField) {
        console.log(`  primary_school field type: ${schoolField.type.name}`);
      }
    }
    ...
    const wizardRankEnum = result.enums!.find((e) => e.name === 'wizard_rank');
