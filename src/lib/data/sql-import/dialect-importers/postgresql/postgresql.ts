...
function preprocessSQL(sqlContent: string): PreprocessResult {
  ...
  for (const stmt of sqlStatements) {
    ...
    if (
      ...
      warnings.push('Policy definitions are not supported and will be skipped');
        ...
        if (defVal.startsWith("'") && defVal.endsWith("'")) {
          ...
          if (funcMatch) {
            ...
            if (pgFunctions.includes(funcName.toLowerCase())) {
              defaultValue = defVal.replace(funcName, funcName.toUpperCase());
            } else {
              defaultValue = defVal;
            }
          } else {
  ...
  for (let i = 0; i < valuesString.length; i++) {
    ...
    if (!inString) {
      ...
      if (char === stringChar) {
        ...
        if (i + 1 < valuesString.length && valuesString[i + 1] === stringChar) {
          currentValue += char;
          i++; // Skip the next quote
        } else {
          // End of string
...
export async function fromPostgres(
  ...
  for (const stmt of statements) {
    if (stmt.type === 'table') {
      ...
      const createTableIndex = stmt.sql.toUpperCase().indexOf('CREATE TABLE');
      const sqlFromCreate =
        createTableIndex >= 0 ? stmt.sql.substring(createTableIndex) : stmt.sql;
  ...
  for (const stmt of statements) {
    if (
      ...
      try {
        ...
        const sqlToParse =
          createIndex > 0 && stmt.sql.substring(0, createIndex).includes('--')
    ...
    if (stmt.type === 'table' && stmt.parsed) {
          ...
          (def: ColumnDefinition | ConstraintDefinition) => {
            if (def.resource === 'column') {
              ...
              const definition = columnDef.definition as Record<
                ...
                unknown
              >;
              let rawDataType = String(definition?.dataType || 'TEXT');
              ...
              if (rawDataType === 'CHARACTER VARYING' && columnName) {
              ...
              const typeLength = definition?.length as number | undefined;
              ...
              if (upperType === 'SERIAL') {
                ...
                normalizedBaseType = normalizePostgreSQLType(rawDataType);
              }
              ...
              if (!isSerialType && !isNormalizedIntegerType) {
                ...
                const precision = columnDef.definition?.precision;
                ...
                if (suffix && Array.isArray(suffix) && suffix.length > 0) {
              ...
              if (columnName) {
                const isPrimaryKey =
                  ...
                  columnDef.definition?.constraint === 'primary key';
                ...
                columns.push({
                  ...
                  nullable: isSerialType
                    ? false
                    : columnDef.nullable?.type !== 'not null',
                  ...
                  increment:
                    isSerialType ||
                    columnDef.auto_increment === 'auto_increment' ||
                    // Check if the SQL contains GENERATED IDENTITY for this column
                    (stmt.sql.toUpperCase().includes('GENERATED') &&
                      stmt.sql.toUpperCase().includes('IDENTITY')),
                });
              }
                ...
                if (Array.isArray(constraintDef.definition)) {
                  constraintDef.definition.forEach(
                    (colDef: ColumnReference) => {
                      const pkColumnName = extractColumnName(colDef);
      ...
      if (tableMatch) {
        ...
        if (tableId) {
          ...
          const columns: SQLColumn[] = extractColumnsFromSQL(stmt.sql);
  ...
  for (const stmt of statements) {
    if (stmt.type === 'alter' && stmt.parsed) {
      ...
      if (alterTableStmt.expr && Array.isArray(alterTableStmt.expr)) {
        alterTableStmt.expr.forEach((expr: AlterTableExprItem) => {
          ...
          if (expr.action === 'alter' && expr.resource === 'column') {
            ...
            if (typeof expr.column === 'object' && 'column' in expr.column) {
            ...
            if (
              ...
              if (column) {
                ...
                // Normalize the type
                let normalizedType = normalizePostgreSQLType(rawDataType);
                ...
                if (
                  ...
                  const precision = definition.length || definition.precision;
            ...
            // Handle ADD COLUMN
          } else if (expr.action === 'add' && expr.resource === 'column') {
            ...
            if (columnName && typeof columnName === 'string') {
              const definition = expr.definition || {};
              const rawDataType = String(definition?.dataType || 'TEXT');
              // console.log('expr:', JSON.stringify(expr, null, 2));

              // Normalize the type
              let normalizedBaseType = normalizePostgreSQLType(rawDataType);
              ...
              if (!isSerialType && !isNormalizedIntegerType) {
                ...
                const suffix = (definition?.suffix as unknown[]) || [];
              ...
              if (isSerialType) {
                nullable = false;
              } else if (expr.nullable && expr.nullable.type === 'not null') {
                nullable = false;
              } else if (
                definition?.nullable &&
              ...
              const isUnique =
                expr.unique === 'unique' || definition?.unique === 'unique';

              // Check for default value
              let defaultValue: string | undefined;
              const defaultVal = expr.default_val || definition?.default_val;
              if (defaultVal && !isSerialType) {
                // Create a temporary columnDef to use the getDefaultValueString function
                const tempColumnDef = {
                  default_val: defaultVal,
                } as ColumnDefinition;
                defaultValue = getDefaultValueString(tempColumnDef);
              }
              ...
              const newColumn: SQLColumn = {
                ...
                increment:
                  isSerialType ||
                  definition?.auto_increment === 'auto_increment' ||
                  (stmt.sql.toUpperCase().includes('GENERATED') &&
                    stmt.sql.toUpperCase().includes('IDENTITY')),
              };
            ...
            if (createDefs.resource === 'column') {
              ...
              if (columnName) {
                ...
                // Add the column to the table if it doesn't already exist
                const tableColumns2 = table.columns as SQLColumn[];
                if (!tableColumns2.some((col) => col.name === columnName)) {
              ...
              if (
                ...
                sourceColumns = createDefs.definition.map(
                  (col: ColumnReference) => extractColumnName(col)
                );
              }

              // Extract target information
              const reference = createDefs.reference_definition;
              if (reference && reference.table && sourceColumns.length > 0) {
                let targetTable = '';
                ...
                if (typeof reference.table === 'object') {
                  if (
                    ...
                    targetTable = reference.table[0].table || '';
                    targetSchema =
                      reference.table[0].schema ||
                      reference.table[0].db ||
                      'public';
                  } else {
                    const tableRef = reference.table as TableReference;
                    targetTable = tableRef.table || '';
                    targetSchema = tableRef.schema || tableRef.db || 'public';
                  }
                } else {
      ...
      if (alterTypeMatch) {
        const schemaName = alterTypeMatch[1] || alterTypeMatch[2] || 'public';
      ...
      if (alterColumnMatch) {
        ...
        if (table && columnName) {
          ...
          if (!tableColumns.some((col) => col.name === columnName)) {
            ...
            const isPrimary = columnDefPart.match(/PRIMARY\s+KEY/i) !== null;
            const isNotNull = columnDefPart.match(/NOT\s+NULL/i) !== null;
            const isUnique = columnDefPart.match(/\bUNIQUE\b/i) !== null;
            // Extract default value
            let defaultValue: string | undefined;
            // Updated regex to handle casting with :: operator
      ...
      if (alterFKMatch) {
        // Extract values from appropriate match groups
        const sourceSchema = alterFKMatch[1] || alterFKMatch[2] || 'public';
        ...
        const targetSchema = alterFKMatch[9] || alterFKMatch[10] || 'public';
        ...
        if (table) {
          ...
          if (
            ...
            columns = createIndexStmt.index_columns
              .map((col: { column?: ColumnReference } | ColumnReference) => {
                const colRef = 'column' in col ? col.column : col;
