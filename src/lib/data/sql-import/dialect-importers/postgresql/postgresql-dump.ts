...
function processUniqueConstraint(
  ...
  try {
    ...
    if (uniqueColumns.length === 1) {
      const column = table.columns.find((c) => c.name === uniqueColumns[0]);
      if (column) {
        column.unique = true;
      }
    }
...
export async function fromPostgresDump(
  ...
  try {
                  ...
                  if (columnName) {
                    ...
                    columns.push({
                      ...
                      default: columnDef.default_val
                        ...
                        : undefined,
                      increment: columnDef.auto_increment === 'auto_increment',
                    });
                    columnsFromAst = true;
                  }
                }
    ...
    for (const statement of alterTableStatements) {
      if (statement.includes('UNIQUE') && !statement.includes('PRIMARY KEY')) {
