  ...
  if (typeof columnObj === 'object') {
    ...
    if (columnObj.column) {
      ...
      if (typeof columnObj.column === 'object') {
        ...
        if (columnObj.column.expr?.value) return columnObj.column.expr.value;
    ...
    if (columnObj.expr) {
      if (columnObj.expr.type === 'default') return columnObj.expr.value || '';
...
export function findTableWithSchemaSupport(
  ...
  if (!table && schemaName) {
    table = tables.find((t) => t.name === tableName && t.schema === schemaName);
  }
