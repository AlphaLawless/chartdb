...
import { extractColumnName, getTypeArgs, parserOpts } from './mysql-common';
...
function extractColumnsFromCreateTable(statement: string): SQLColumn[] {
  ...
  for (const columnLine of columnLines) {
    ...
    if (columnNameMatch) {
      ...
      const increment = definition.toUpperCase().includes('AUTO_INCREMENT');
...
export async function fromMySQL(sqlContent: string): Promise<SQLParserResult> {
  ...
  try {
    ...
    for (const statement of statements) {
      ...
      if (trimmedStmt.toUpperCase().startsWith('CREATE TABLE')) {
        try {
          ...
          if (
            ...
            if (typeof createTableStmt.table === 'object') {
              if (
                ...
                tableName = createTableStmt.table[0].table || '';
              } else {
                ...
                createTableStmt.create_definitions.forEach(
                  (def: ColumnDefinition | ConstraintDefinition) => {
                    if (def.resource === 'column') {
                      ...
                      let columnName = extractColumnName(columnDef.column);
                      ...
                      columns.push({
                        ...
                        unique:
                          ...
                          columnDef.definition?.constraint === 'unique',
                        typeArgs: getTypeArgs(columnDef.definition),
                        default: columnDef.default_val
                          ? buildSQLFromAST(columnDef.default_val)
                          : undefined,
                      ...
                      if (constraintDef.constraint_type === 'primary key') {
                        if (Array.isArray(constraintDef.definition)) {
                          const pkColumns = constraintDef.definition
                            ...
                            .map((colDef) =>
                              extractColumnName(colDef).replace(/`/g, '')
                            );
                      ...
                      // Handle UNIQUE constraint
                        ...
                        const uniqueColumns = Array.isArray(
                          ...
                          ? constraintDef.definition.map((colDef) =>
                              extractColumnName(colDef).replace(/`/g, '')
                            )
                          : (constraintDef.definition?.columns || []).map(
                              (col) =>
                                typeof col === 'string'
                                  ? col.replace(/`/g, '')
                        ...
                        if (uniqueColumns.length > 0) {
                          indexes.push({
                            name: constraintDef.constraint_name
                              ? constraintDef.constraint_name.replace(/`/g, '')
                      ...
                      // Handle FOREIGN KEY constraints
                        ...
                        if (Array.isArray(constraintDef.definition)) {
                          sourceColumns = constraintDef.definition.map(
                            (col) => {
                              const colName = extractColumnName(col).replace(
                                /`/g,
                                ''
                              );
                              return colName;
                        ...
                        if (reference && sourceColumns.length > 0) {
                          ...
                          if (
                            ...
                            targetColumns = reference.columns.map((col) => {
                              const colName =
                                typeof col === 'string'
                                  ? col.replace(/`/g, '')
                                  : extractColumnName(col).replace(/`/g, '');
                            ...
                            targetColumns = reference.definition.map((col) => {
                              const colName = extractColumnName(col).replace(
                                /`/g,
                                ''
                              );
                          ...
                          if (targetTable && targetColumns.length > 0) {
          ...
          console.error('Error parsing CREATE TABLE statement:', parseError);
          ...
          if (tableMatch) {
            ...
            const extractedColumns = extractColumnsFromCreateTable(trimmedStmt);
            if (extractedColumns.length > 0) {
              tables.push({
                id: tableId,
                name: tableName,
      ...
      if (
        ...
        try {
          ...
          if (!sourceTableId && databaseName) {
            sourceTableId = tableMap[`${databaseName}.${sourceTable}`];
          }
          if (!sourceTableId) {
            continue;
          }
          ...
          // Extract source columns
          const sourceColMatch = statement.match(/FOREIGN KEY\s*\(([^)]+)\)/i);
          ...
          if (!targetTableId && targetDatabase) {
            targetTableId = tableMap[`${targetDatabase}.${targetTable}`];
          }
          ...
          for (
            ...
            const fk: SQLForeignKey = {
              name: constraintName || `${sourceTable}_${sourceColumns[i]}_fkey`,
              sourceTable,
              sourceColumn: sourceColumns[i],
              targetTable,
              targetColumn: targetColumns[i],
    ...
    throw new Error(`Error parsing MySQL dump: ${(error as Error).message}`);
  }
}
...
function findForeignKeysUsingRegex(
  ...
  for (const stmt of createTableStatements) {
    ...
    const pkColumns = pkMatch
      ? pkMatch[1].split(',').map((col) => col.trim().replace(/[`'"]/g, ''))
      : [];
