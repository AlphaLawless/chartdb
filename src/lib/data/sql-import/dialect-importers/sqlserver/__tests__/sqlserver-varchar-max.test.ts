...
describe('SQL Server varchar(max) and nvarchar(max) preservation', () => {
  it('should preserve varchar(max) and nvarchar(max) in column definitions', async () => {
    ...
    const descriptionCol = table.columns.find((c) => c.name === 'Description');
  ...
  it('should convert varchar(max) to characterMaximumLength field in diagram', async () => {
    ...
    const incantationField = table.fields.find((f) => f.name === 'Incantation');
    ...
    const powerLevelField = table.fields.find((f) => f.name === 'PowerLevel');
  ...
  it('should handle mixed varchar types with schema and relationships', async () => {
    ...
    const summaryCol = booksTable?.columns.find((c) => c.name === 'Summary');
    expect(summaryCol?.typeArgs).toBe('max');

    const fullTextCol = booksTable?.columns.find((c) => c.name === 'FullText');
  ...
  it('should handle complex table with various SQL Server features including varchar(max)', async () => {
    ...
    const reportCol = table.columns.find((c) => c.name === 'DetailedReport');
    ...
    const scoreCol = table.columns.find((c) => c.name === 'PerformanceScore');
