...
function preprocessSQLServerScript(sqlContent: string): string {
  ...
  sqlContent = sqlContent.replace(/\bdatetime2\s*\(\s*\d+\s*\)/gi, 'datetime2');
...
function parseCreateTableManually(
  ...
  for (const part of parts) {
    ...
    if (part.match(/^\s*FOREIGN\s+KEY/i)) {
      ...
      if (fkMatch) {
        const [, sourceCol, targetSchema = 'dbo', targetTable, targetCol] =
          fkMatch;
        relationships.push({
      ...
      if (constraintMatch) {
        ...
        if (constraintType.match(/PRIMARY\s+KEY/i)) {
          ...
          if (uniqueColumnsMatch) {
            const uniqueColumns = uniqueColumnsMatch[1].split(',').map((c) =>
              c
                .trim()
                .replace(/\[|\]|\s+(ASC|DESC)/gi, '')
                .trim()
    ...
    if (columnMatch) {
      ...
      if (colName && !colName.match(/^(PRIMARY|FOREIGN|UNIQUE|CHECK)$/i)) {
          ...
          if (fkWithColumnMatch) {
            ...
            if (srcCol.toLowerCase() === colName.toLowerCase()) {
              ...
              relationships.push({
                ...
                targetTable: targetTable || targetSchema,
                targetSchema: targetTable ? targetSchema || 'dbo' : 'dbo',
                targetColumn: targetCol.trim().replace(/\[|\]/g, ''),
                sourceTableId: tableId,
                targetTableId: '', // Will be filled later
              });
...
export async function fromSQLServer(
  ...
  try {
    ...
    for (const stmt of createIndexStatements) {
      ...
      if (indexMatch) {
        const [, unique, indexName, schema = 'dbo', tableName, columnsStr] =
          indexMatch;
...
function processCreateIndex(
  ...
  // Create the index
  const indexName = stmt.index || `idx_${tableName}_${indexColumns.join('_')}`;
    ...
    if (action === 'add' && expr.resource === 'constraint') {
      ...
      if (
        ...
        if (
          ...
          const targetColumnName = extractColumnName(reference.columns[0]);
