        ...
        func.args.value
          .map((v: SQLASTArg) => {
            if (v.type === 'column_ref')
              return quoteIdentifier((v as SQLColumnRef).column, dbType);
            if (
...
export function mapSQLTypeToGenericType(
  ...
  if (databaseType) {
    ...
    if (typeMapping) {
      const foundType = genericDataTypes.find((t) => t.id === typeMapping);
    ...
    const fields: DBField[] = table.columns.map((column) => {
      ...
      if (sourceDatabaseType === DatabaseType.SQLITE) {
        ...
        if (normalizedType === 'integer' || normalizedType === 'int') {
          ...
          mappedType = mapSQLTypeToGenericType(column.type, sourceDatabaseType);
        }
      }
      ...
      if (column.typeArgs) {
        ...
        if (typeof column.typeArgs === 'string') {
          if (
            (field.type.id === 'varchar' || field.type.id === 'nvarchar') &&
            column.typeArgs === 'max'
          ) {
        ...
        // Handle object typeArgs (standard format)
          ...
          if (
            ...
            field.characterMaximumLength = typeArgsObj.length.toString();
          }
  ...
  parserResult.relationships.forEach((rel) => {
    ...
    if (!sourceTable || !targetTable) {
      console.warn('Relationship refers to non-existent table:', {
        ...
        availableTables: tables.map((t) => `${t.schema || ''}.${t.name}`),
      });
      return;
    }
