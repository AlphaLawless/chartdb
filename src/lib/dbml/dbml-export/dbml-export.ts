...
const fixProblematicFieldNames = (diagram: Diagram): Diagram => {
  ...
  const fixedRelationships =
    diagram.relationships?.map((rel) => {
      const relationTable = diagram.tables?.find((t) => t.name === 'relation');
      if (!relationTable) return rel;

      const newRel = { ...rel };

      // Fix relationships that were pointing to the 'from' field
      const fromField = relationTable.fields.find((f) => f.name === 'from');
...
export const sanitizeSQLforDBML = (sql: string): string => {
  ...
  sanitized = sanitized.replace(
    ...
    (match, columnList) => {
      const columns = columnList.split(',').map((col: string) => col.trim());
  ...
  sanitized = sanitized.replace(/DEFAULT\s+([A-Z])(?=\s|,|$)/g, "DEFAULT '$1'");
...
const convertToInlineRefs = (dbml: string): string => {
  ...
  while (currentPos < dbml.length) {
    ...
    const openBracePos = tableStartMatch.index + tableStartMatch[0].length - 1;
    let braceCount = 1;
    ...
    if (braceCount === 0) {
      ...
      const fullMatch = dbml.substring(tableStartMatch.index, contentEnd + 1);
      ...
      newContent = newContent.replace(
        ...
        (lineMatch, fieldPart, existingBrackets, commentPart) => {
          ...
          if (existingBrackets) {
            ...
            let bracketMatch;
            while ((bracketMatch = bracketPattern.exec(lineMatch)) !== null) {
...
const appendRenameComments = (
  ...
  sqlRenamedTables.forEach((originalName, newName) => {
    ...
    const table = finalDiagramForExport.tables?.find((t) => t.name === newName);
...
const fixTableBracketSyntax = (dbml: string): string => {
  ...
  return dbml.replace(/Table\s+\[([^\]]+)\]\.\[([^\]]+)\]/g, 'Table "$1"."$2"');
};
  ...
  tables.forEach((table) => {
    ...
    if (pkIndex?.name) {
      ...
      if (primaryKeyFields.length >= 1) {
        ...
        const columnList = primaryKeyFields.map((f) => f.name).join(', ');
  ...
  tablesByName.forEach((tablesGroup, tableName) => {
    if (tablesGroup.length === 1) {
      ...
      result = result.replace(noSchemaTablePattern, (match) => {
        ...
        const correspondingTable = tablesNeedingSchema[noSchemaMatchIndex];
        noSchemaMatchIndex++;
...
export function generateDBMLFromDiagram(diagram: Diagram): DBMLExportResult {
  ...
  const sanitizedTables =
    diagram.tables?.map((table) => {
      const validFields = table.fields.filter((field) => field.name !== '');
      return {
        ...table,
        fields: validFields,
      };
  ...
  const filteredDiagram: Diagram = {
    ...
    relationships:
      diagram.relationships?.filter((rel) => {
        ...
        return (
          sourceTable && targetTable && sourceFieldExists && targetFieldExists
        );
      }) ?? [],
  };
