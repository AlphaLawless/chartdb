...
const mapDBMLTypeToDataType = (
  ...
  if (options?.enums) {
    const enumDef = options.enums.find((e) => {
      ...
      const enumFullName = e.schema ? `${e.schema}.${enumName}` : enumName;
      return normalizedType === enumName || normalizedType === enumFullName;
    });
...
export const importDBMLToDiagram = async (
  ...
  try {
              ...
              table.indexes?.map((dbmlIndex) => {
                ...
                if (typeof dbmlIndex.columns === 'string') {
                  ...
                  if (dbmlIndex.columns.includes('(')) {
                    ...
                    indexColumns = columnsStr.split(',').map((c) => c.trim());
                  } else {
                  ...
                  indexColumns = dbmlIndex.columns.map((col) => {
                    if (typeof col === 'string') {
                      // @ts-expect-error "columns" can be a string in some DBML versions
                      return col.trim();
                    } else if (typeof col === 'object' && 'value' in col) {
    ...
    const tables: DBTable[] = extractedData.tables.map((table, index) => {
      ...
      if (compositePKFields.length > 0 && !compositePKIndexName) {
        table.indexes?.forEach((dbmlIndex) => {
          if (
            ...
            if (indexColumns.every((col, i) => col === compositePKFields[i])) {
              compositePKIndexName = dbmlIndex.name;
            }
          }
        });
          ...
          ?.filter((dbmlIndex) => {
            ...
            if (
              ...
              dbmlIndex.columns.every((col, i) => {
                const colName = typeof col === 'string' ? col : col.value;
      ...
      if (compositePKFields.length >= 1 && compositePKIndexName) {
        const pkFieldIds = compositePKFields.map((columnName) => {
          const field = fields.find((f) => f.name === columnName);
          if (!field) {
            throw new Error(`PK references non-existent column: ${columnName}`);
          }
          return field.id;
        });
      ...
      if (table.note) {
        ...
        } else if (typeof table.note === 'object' && 'value' in table.note) {
    ...
    const relationships: DBRelationship[] = extractedData.refs.map((ref) => {
    ...
    const customTypes: DBCustomType[] = extractedData.enums.map((enumDef) => {
      ...
      return {
        id: generateId(),
        schema: typeof enumDef.schema === 'string' ? enumDef.schema : undefined,
        name: enumDef.name,
