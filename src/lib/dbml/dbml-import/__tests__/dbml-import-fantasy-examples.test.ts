...
Table ranks {
  ...
  max_spell_level integer [not null]
      ...
      const emailField = wizardsTable?.fields.find((f) => f.name === 'email');
      ...
      const mentorRelationship = diagram.relationships?.find(
        (r) =>
          r.sourceTableId ===
            diagram.tables?.find((t) => t.name === 'apprentices')?.id &&
          r.targetTableId ===
...
Note marketplace_note {
  'This marketplace handles both standard purchases and barter trades'
      ...
      const typeField = artifactsTable?.fields.find((f) => f.name === 'type');
...
Table projects {
  ...
  priority enum // inline enum without values - will be converted to varchar
      ...
      const typeField = employeesTable?.fields.find((f) => f.name === 'type');
      ...
      const projectsTable = diagram.tables?.find((t) => t.name === 'projects');
...
Table "public_3"."comments" {
  ...
  Indexes {
    id [unique, name: "public_3_index_1"]
      ...
      expect(usersTable?.fields.find((f) => f.name === 'id')?.primaryKey).toBe(
        true
      );
      expect(usersTable?.fields.find((f) => f.name === 'id')?.type.id).toBe(
        'varchar'
      );
      expect(usersTable?.fields.find((f) => f.name === 'name')?.type.id).toBe(
        'varchar'
      );
      expect(usersTable?.fields.find((f) => f.name === 'email')?.type.id).toBe(
        'varchar'
      );
      ...
      const idFieldId = commentsTable?.fields.find((f) => f.name === 'id')?.id;
      ...
      const allOneToMany = diagram.relationships?.every(
        (r) => r.sourceCardinality === 'one' && r.targetCardinality === 'many'
      );
      ...
      const relationshipsHaveSchemas = diagram.relationships?.every(
        (r) => r.sourceSchema !== undefined && r.targetSchema !== undefined
      );
...
Table orders {
  ...
  total numeric(10,2) [note: 'Order total including tax']
      ...
      const totalField = ordersTable?.fields.find((f) => f.name === 'total');
