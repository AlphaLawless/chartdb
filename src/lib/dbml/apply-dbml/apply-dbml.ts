...
const buildSourceMappings = (sourceDiagram: Diagram) => {
  ...
  sourceDiagram.dependencies?.forEach((dependency) => {
    const key = createObjectKeyFromDependency(dependency, sourceIdToDataMap);
    if (key) {
...
const updateDependencies = (
  ...
  return targetDependencies.map((targetDep) => {
    ...
    const sourceDep = sourceDependencies.find((srcDep) => {
      ...
      const srcDependentTableId = idMappings.tables[targetDep.dependentTableId];
...
export const applyDBMLChanges = ({
  ...
  const sortedRelationships = [...updatedRelationships].sort((a, b) => {
    ...
    const sourceRelA = sourceDiagram.relationships?.find((r) => r.id === a.id);
    const sourceRelB = sourceDiagram.relationships?.find((r) => r.id === b.id);
