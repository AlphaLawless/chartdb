...
import { type DBField, dbFieldSchema } from './db-field';
import { type DBIndex, dbIndexSchema } from './db-index';
...
export const adjustTablePositions = ({
  ...
  areas.forEach((area) => {
    ...
    if (tablesToReposition.length > 0) {
      ...
      positionTablesWithinArea(tablesToReposition, areaRelationships, area);
    }
  ...
  if (freeTables.length > 0) {
    ...
    const freeRelationships = relationships.filter((rel) => {
      const sourceIsFree = freeTables.some((t) => t.id === rel.sourceTableId);
      const targetIsFree = freeTables.some((t) => t.id === rel.targetTableId);
