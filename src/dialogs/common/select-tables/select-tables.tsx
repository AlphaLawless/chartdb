import { AlertCircle, Check, Search, Table, View, X } from 'lucide-react';
  ...
  const allTables = useMemo(() => {
    ...
    databaseMetadata?.views?.forEach((view) => {
      ...
      tables.push({
        ...
        schema,
        tableName: viewName,
        fullName: schema === 'default' ? viewName : `${schema}.${viewName}`,
        type: 'view',
      });
    });
        ...
        <PaginationContent>
          ...
          <PaginationItem>
            <PaginationNext
              ...
              className={cn(
                'cursor-pointer',
                (currentPage >= totalPages || filteredTables.length === 0) &&
        ...
        <div className="flex h-full flex-col space-y-4">
          ...
          {allTables.length > MAX_TABLES_IN_DIAGRAM ? (
            ...
            >
              ...
              <span>
                Due to performance limitations, you can import a maximum of{' '}
                {MAX_TABLES_IN_DIAGRAM} tables.
              </span>
            </div>
          ) : null}
          {/* Search Input */}
            ...
            <div className="flex flex-col items-center gap-3 text-sm sm:flex-row sm:items-center sm:gap-4">
              <div className="flex flex-col items-center gap-1 sm:flex-row sm:items-center sm:gap-4">
                <span className="text-center font-medium">
                  {selectedTables.size} /{' '}
                  {Math.min(MAX_TABLES_IN_DIAGRAM, allTables.length)} items
                  selected
                </span>
              </div>
              ...
              <div className="flex items-center gap-3 sm:border-x sm:px-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <View className="size-4" strokeWidth={1.5} />
                  <span>views</span>
                </div>
              </div>
              ...
              <span className="hidden text-muted-foreground sm:inline">
                {filteredTables.length}{' '}
                {filteredTables.length === 1 ? 'result' : 'results'} found
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              {hasSearchResults && (
                <>
                  ...
                  {!searchTerm && selectedTables.size === 0 && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={handleTogglePageSelection}
                      disabled={paginatedTables.length === 0}
                    >
                      {allVisibleSelected ? 'Deselect' : 'Select'} page
                    </Button>
                  )}
                  {/* Show Select all button when there are unselected tables */}
                  {canSelectAllFiltered && selectedTables.size === 0 && (
                    <Button
                      variant="outline"
                      size="sm"
                      ...
                      title={(() => {
                        const unselectedCount = filteredTables.filter(
                          (table) => !selectedTables.has(table.key)
                        ).length;
                        const remainingCapacity =
                          MAX_TABLES_IN_DIAGRAM - selectedTables.size;
                        if (unselectedCount > remainingCapacity) {
                      ...
                      <div
                        ...
                        className={cn(
                          'flex items-center gap-3 rounded-md px-3 py-2 text-sm transition-colors',
                          {
                            ...
                            'bg-muted hover:bg-muted/80': isSelected,
                            'hover:bg-accent': !isSelected && !isDisabled,
                          }
                        )}
                      >
                        <Checkbox
