...
export const SmartQueryInstructions: React.FC<SmartQueryInstructionsProps> = ({
  ...
  const databaseClients = useMemo(
    () => [
      ...
      ...(databaseEdition ? databaseEditionToClientsMap[databaseEdition] : []),
    ],
      ...
      <div className="flex flex-col gap-1">
        ...
        {databaseClients.length > 0 ? (
          <Tabs
            ...
            onValueChange={(value) => {
              setDatabaseClient(
                value === 'dbclient' ? undefined : (value as DatabaseClient)
              );
            }}
          >
            <div className="flex flex-1">
              <TabsList className="h-8 justify-start rounded-none rounded-t-sm ">
                <TabsTrigger value="dbclient" className="h-6 w-20">
                  DB Client
                </TabsTrigger>
