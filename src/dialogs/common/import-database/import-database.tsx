    ...
    () => (
      <div className="flex size-full flex-col gap-1 overflow-hidden rounded-md border p-1">
        <div className="w-full text-center text-xs text-muted-foreground">
          {importMethod === 'query' ? 'Smart Query Output' : 'SQL Script'}
        </div>
          ...
          {isDesktop ? (
            <ZoomableImage src="/load-new-db-instructions.gif">
              <Button type="button" variant="link">
                {t('new_diagram_dialog.import_database.instructions_link')}
              </Button>
            </ZoomableImage>
          ) : null}
        </div>
          ...
          {showCheckJsonButton ? (
            ...
            >
              {isCheckingJson ? (
                ...
                t('new_diagram_dialog.import_database.check_script_result')
              )}
            </Button>
            ...
            >
              {isAutoFixing ? <Spinner size="small" /> : 'Try auto-fix'}
            </Button>
          ) : keepDialogAfterImport ? (
            <Button
              type="button"
