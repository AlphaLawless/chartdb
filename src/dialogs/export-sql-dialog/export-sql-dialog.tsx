...
export const ExportSQLDialog: React.FC<ExportSQLDialogProps> = ({
  ...
  const [isScriptLoading, setIsScriptLoading] = React.useState<boolean>(false);
  ...
  const exportSQLScript = useCallback(async () => {
    const filteredDiagram: Diagram = {
      ...
      dependencies: currentDiagram.dependencies?.filter((dep) => {
        const table = currentDiagram.tables?.find((t) => t.id === dep.tableId);
        ...
        <div className="flex items-center justify-center gap-1">
          ...
          <Label className="text-lg">
            {t('export_sql_dialog.loading.text', {
              databaseType: databaseTypeToLabelMap[targetDatabaseType],
            })}
          </Label>
        </div>
        <div className="flex items-center justify-center gap-1">
