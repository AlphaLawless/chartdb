    ...
    async (content: string) => {
      ...
      try {
        ...
        if (parsedError) {
          ...
          setErrorMessage(e instanceof Error ? e.message : JSON.stringify(e));
        }
      }
    },
    [clearDecorations, t]
      ...
      const tableIdsToRemove = tables
        .filter((table) =>
          importedDiagram.tables?.some(
            (t: DBTable) => t.name === table.name && t.schema === table.schema
          )
        )
        .map((table) => table.id);
      ...
      const relationshipIdsToRemove = relationships
        .filter((relationship) => {
          const sourceTable = tables.find(
            (table: DBTable) => table.id === relationship.sourceTableId
          );
          const targetTable = tables.find(
            (table: DBTable) => table.id === relationship.targetTableId
          );
        ...
        <DialogInternalContent>
          <Suspense fallback={<Spinner />}>
            <Editor
              ...
              theme={effectiveTheme === 'dark' ? 'dbml-dark' : 'dbml-light'}
