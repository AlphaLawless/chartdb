...
export const OpenDiagramDialog: React.FC<OpenDiagramDialogProps> = ({
  ...
  const fetchDiagrams = useCallback(async () => {
    ...
    setDiagrams(
      diagrams.sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime())
    );
  }, [listDiagrams]);
  ...
  const handleRowKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLTableRowElement>) => {
      ...
      const selectionIndexAttr = element.getAttribute('data-selection-index');
            ...
            <Table>
              <TableHeader className="sticky top-0 bg-background">
                <TableRow>
                  <TableHead />
                  <TableHead>
                    {t('open_diagram_dialog.table_columns.name')}
                  </TableHead>
                  <TableHead className="hidden items-center sm:inline-flex">
                    {t('open_diagram_dialog.table_columns.created_at')}
                  </TableHead>
                  <TableHead>
                    {t('open_diagram_dialog.table_columns.last_modified')}
                  </TableHead>
                  <TableHead className="text-center">
                    {t('open_diagram_dialog.table_columns.tables_count')}
                  </TableHead>
                  <TableHead />
                </TableRow>
              </TableHeader>
              <TableBody>
                {diagrams.map((diagram, index) => (
                  <TableRow
                    ...
                    tabIndex={0}
                    onFocus={() => onFocusHandler(diagram.id)}
                  ...
                  >
                    ...
                    <TableCell>{diagram.updatedAt.toLocaleString()}</TableCell>
                    ...
                    <TableCell className="items-center p-0 pr-1 text-right">
                      <DiagramRowActionsMenu
                        ...
                        numberOfDiagrams={diagrams.length}
