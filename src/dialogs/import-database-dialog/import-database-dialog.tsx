  ...
  const importDatabase = useCallback(async () => {
    ...
    if (importMethod === 'ddl') {
      ...
      diagram = await loadFromDatabaseMetadata({
        ...
        databaseMetadata,
        databaseEdition:
          databaseEdition?.trim().length === 0 ? undefined : databaseEdition,
      });
    }
    ...
    const newTablesNumber = diagram.tables?.filter(
      (table) =>
        !tables.some((t) => t.name === table.name && t.schema === table.schema)
    ).length;
          ...
          {tableIdsToRemove.length > 0 && (
            <div className="!m-0 text-red-500">
              <Trans
                ...
                values={{
                  tablesOverrideNumber: tableIdsToRemove.length,
                }}
                components={{
                  bold: <span className="font-bold" />,
                }}
    ...
    setNodes((nodes) =>
      nodes.map((node) => ({
        ...node,
        selected:
          diagram.tables?.some((table) => table.id === node.id) ?? false,
      }))
    );
