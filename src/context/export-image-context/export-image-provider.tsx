  ...
  const exportImage: ExportImageContext['exportImage'] = useCallback(
    async (type, { includePatternBG, transparent, scale }) => {
      ...
      setTimeout(async () => {
        ...
        const markerDefs = document.querySelector('.marker-definitions defs');
        ...
        if (includePatternBG) {
          ...
          const dotColor = effectiveTheme === 'light' ? '#92939C' : '#777777';
        ...
        backgroundRect.setAttribute('x', String(-viewport.x - bgPadding));
        backgroundRect.setAttribute('y', String(-viewport.y - bgPadding));
        ...
        viewportElement.insertBefore(tempSvg, viewportElement.firstChild);
        ...
        try {
          ...
          const initialDataUrl = await imageCreateFn(viewportElement, {
            backgroundColor: getBackgroundColor(effectiveTheme, transparent),
            width: reactFlowBounds.width,
            height: reactFlowBounds.height,
            style: {
              width: `${reactFlowBounds.width}px`,
          ...
          await new Promise((resolve) => {
            diagramImage.onload = async () => {
              ...
              // Calculate logo size
              const logoHeight = Math.max(24, Math.floor(canvas.width * 0.024));
              const padding = Math.max(12, Math.floor(logoHeight * 0.5));
              ...
              await new Promise((resolve) => {
                logoImage.onload = () => {
                  ...
                  const logoWidth =
                    (logoImage.width / logoImage.height) * logoHeight;
