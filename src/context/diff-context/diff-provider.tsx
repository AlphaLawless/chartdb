...
export const DiffProvider: React.FC<React.PropsWithChildren> = ({
  ...
  const [originalDiagram, setOriginalDiagram] = React.useState<Diagram | null>(
    null
  );
  ...
  const generateNewFieldsMap = useCallback(
    ({ diffMap, newDiagram }: { diffMap: DiffMap; newDiagram: Diagram }) => {
  ...
  const calculateDiff: DiffContext['calculateDiff'] = useCallback(
    ...
    [events, generateDiffCalculatedData]
  );
  ...
  const checkIfNewTable = useCallback<DiffContext['checkIfNewTable']>(
    ({ tableId }) => {
      ...
      return diffMap.has(tableKey) && diffMap.get(tableKey)?.type === 'added';
    },
    [diffMap]
  );
  ...
  const checkIfTableRemoved = useCallback<DiffContext['checkIfTableRemoved']>(
    ({ tableId }) => {
      ...
      return diffMap.has(tableKey) && diffMap.get(tableKey)?.type === 'removed';
    },
    [diffMap]
  );
  ...
  const checkIfFieldRemoved = useCallback<DiffContext['checkIfFieldRemoved']>(
    ({ fieldId }) => {
      ...
      return diffMap.has(fieldKey) && diffMap.get(fieldKey)?.type === 'removed';
    },
    [diffMap]
  );
  ...
  const checkIfNewField = useCallback<DiffContext['checkIfNewField']>(
    ({ fieldId }) => {
      ...
      return diffMap.has(fieldKey) && diffMap.get(fieldKey)?.type === 'added';
    },
    [diffMap]
  );
  ...
  const getFieldNewPrecision = useCallback<DiffContext['getFieldNewPrecision']>(
    ({ fieldId }) => {
      const fieldKey = getDiffMapKey({
        diffObject: 'field',
        objectId: fieldId,
