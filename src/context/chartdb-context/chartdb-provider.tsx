...
import { defaultAreaColor, defaultTableColor, viewColor } from '@/lib/colors';
...
> = ({ children, diagram, readonly: readonlyProp }) => {
  ...
  const { addUndoAction, resetRedoStack, resetUndoStack } = useRedoUndoStack();
        ...
        ? [
            ...
            .map(
              (schema): DBSchema => ({
                ...
                name: schema,
                tableCount: tables.filter((table) => table.schema === schema)
                  .length,
              })
            )
        : [],
  ...
  const updateDatabaseType: ChartDBContext['updateDatabaseType'] = useCallback(
    async (databaseType) => {
  ...
  const updateDiagramName: ChartDBContext['updateDiagramName'] = useCallback(
    ...
    [db, diagramId, addUndoAction, diagramName, resetRedoStack]
  );
  ...
  const addTables: ChartDBContext['addTables'] = useCallback(
    async (tablesToAdd: DBTable[], options = { updateHistory: true }) => {
      ...
      await Promise.all([
        db.updateDiagram({ id: diagramId, attributes: { updatedAt } }),
        ...tablesToAdd.map((table) => db.addTable({ diagramId, table })),
      ]);
    ...
    async (ids, options) => {
      ...
      setRelationships((relationships) =>
        relationships.filter(
          (relationship) =>
            !relationshipsToRemove.some((r) => r.id === relationship.id)
        )
      );
      ...
      setDependencies((dependencies) =>
        dependencies.filter(
          (dependency) =>
            !dependenciesToRemove.some((d) => d.id === dependency.id)
        )
      );

      setTables((tables) => tables.filter((table) => !ids.includes(table.id)));
  ...
  const updateTable: ChartDBContext['updateTable'] = useCallback(
    ...
    [db, addUndoAction, resetRedoStack, getTable, diagramId, events]
  );
  ...
  const updateTablesState: ChartDBContext['updateTablesState'] = useCallback(
    async (
      ...
      const updateTables = (prevTables: DBTable[]) => {
        ...
        return prevTables
          .map((prevTable) => {
            ...
            return updatedTable ? { ...prevTable, ...updatedTable } : prevTable;
          })
          .filter((prevTable) =>
      ...
      for (const dependency of dependenciesToRemove) {
        promises.push(db.deleteDependency({ diagramId, id: dependency.id }));
      }
  ...
  const updateField: ChartDBContext['updateField'] = useCallback(
    ...
    [db, diagramId, addUndoAction, resetRedoStack, getField]
  );
  ...
  const removeField: ChartDBContext['removeField'] = useCallback(
    ...
    [db, diagramId, addUndoAction, resetRedoStack, getField, getTable, events]
  );
  ...
  const addField: ChartDBContext['addField'] = useCallback(
    ...
    [db, diagramId, addUndoAction, resetRedoStack, events, getTable]
  );
  ...
  const removeIndex: ChartDBContext['removeIndex'] = useCallback(
    ...
    [db, diagramId, addUndoAction, resetRedoStack, getIndex]
  );
  ...
  const addRelationship: ChartDBContext['addRelationship'] = useCallback(
    async (relationship: DBRelationship, options = { updateHistory: true }) => {
      return addRelationships([relationship], options);
    },
    [addRelationships]
  );

  const createRelationship: ChartDBContext['createRelationship'] = useCallback(
    async ({ sourceTableId, targetTableId, sourceFieldId, targetFieldId }) => {
  ...
  const removeRelationships: ChartDBContext['removeRelationships'] =
    useCallback(
      async (ids: string[], options = { updateHistory: true }) => {
        ...
        setRelationships((relationships) =>
          relationships.filter((relationship) => !ids.includes(relationship.id))
        );
  ...
  const removeRelationship: ChartDBContext['removeRelationship'] = useCallback(
    async (id: string, options = { updateHistory: true }) => {
  ...
  const updateRelationship: ChartDBContext['updateRelationship'] = useCallback(
    async (
      id: string,
      ...
      setRelationships((relationships) =>
        relationships.map((r) => (r.id === id ? { ...r, ...relationship } : r))
      );
  ...
  const addDependencies: ChartDBContext['addDependencies'] = useCallback(
    async (dependencies: DBDependency[], options = { updateHistory: true }) => {
  ...
  const removeDependencies: ChartDBContext['removeDependencies'] = useCallback(
    async (ids: string[], options = { updateHistory: true }) => {
  ...
  const updateDependency: ChartDBContext['updateDependency'] = useCallback(
    async (
      ...
      setDependencies((dependencies) =>
        dependencies.map((d) => (d.id === id ? { ...d, ...dependency } : d))
      );
  ...
  const loadDiagramFromData: ChartDBContext['loadDiagramFromData'] =
    useCallback(
      ...
      [events, resetRedoStack, resetUndoStack]
    );
  ...
  const addCustomTypes: ChartDBContext['addCustomTypes'] = useCallback(
    async (customTypes: DBCustomType[], options = { updateHistory: true }) => {
  ...
  const removeCustomTypes: ChartDBContext['removeCustomTypes'] = useCallback(
    async (ids, options = { updateHistory: true }) => {
      ...
      setCustomTypes((types) => types.filter((type) => !ids.includes(type.id)));
