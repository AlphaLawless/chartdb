...
export const DiagramFilterProvider: React.FC<React.PropsWithChildren> = ({
    ...
    const loadFilterFromStorage = async (diagramId: string) => {
      if (diagramId) {
        ...
        if (!filterToSet) {
          ...
          filterToSet =
            schemas.length > 1 ? { schemaIds: [schemas[0].id] } : {};
        }
        ...
        setFilter((prev) => {
          ...
          if (isSchemaVisible) {
            ...
            if (!currentSchemaIds) {
              ...
              newSchemaIds = allSchemasIds.filter((id) => id !== schemaId);
            } else {
              // Remove this schema from the filter
              newSchemaIds = currentSchemaIds.filter((id) => id !== schemaId);
            }
            ...
            if (prev.tableIds) {
              const schemaTableIds = allTables
                .filter((table) => table.schemaId === schemaId)
                .map((table) => table.id);
              newTableIds = [...new Set([...prev.tableIds, ...schemaTableIds])];
            }
          }
          ...
          return reduceFilter(
            ...
            {
              databaseWithSchemas: databasesWithSchemas.includes(databaseType),
            }
          );
        });
      },
    ...
    (tableId: string) => {
      setFilter((prev) => {
        ...
        if (isTableVisible) {
          ...
          newTableIds = [...new Set([...(currentTableIds || []), tableId])];
        }
          ...
          if (isTableVisible) {
            ...
            if (
              ...
              const otherTablesFromSchema = allTables
                .filter(
                  (t) => t.schemaId === tableInfo.schemaId && t.id !== tableId
                )
                .map((t) => t.id);
              ...
              newTableIds = prev.tableIds.filter((id) => id !== tableId);
  ...
  const schemasDisplayed: DiagramFilterContext['schemasDisplayed'] =
    useMemo(() => {
      ...
      return schemas.filter((schema) => displayedSchemaIds.has(schema.id));
