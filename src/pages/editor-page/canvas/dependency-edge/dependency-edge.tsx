  ...
  const edgeNumber = useMemo(
    () =>
      dependencies
        .filter(
          (dep) =>
            (dep.tableId === target && dep.dependentTableId === source) ||
            (dep.tableId === source && dep.dependentTableId === target)
        )
  ...
  const [edgePath] = useMemo(
    () =>
      getSmoothStepPath({
        ...
        borderRadius: 14,
        sourcePosition: sourceSide === 'top' ? Position.Top : Position.Bottom,
        targetPosition: targetSide === 'top' ? Position.Top : Position.Bottom,
