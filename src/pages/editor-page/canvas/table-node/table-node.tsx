...
export const TableNode: React.FC<NodeProps<TableNodeType>> = React.memo(
  ({
    ...
    const [editModeInitialFieldCount, setEditModeInitialFieldCount] = useState<
      number | null
    >(null);
    ...
    }, [checkIfTableHasChange, checkIfNewTable, checkIfTableRemoved, table.id]);
      ...
      () =>
        cn(
          ...
          isOverlapping
            ...
            : '',
          !highlightOverlappingTables && isOverlapping ? 'animate-scale' : '',
          highlightOverlappingTables && isOverlapping ? 'animate-scale-2' : '',
          hasHighlightedCustomType
    ...
    return (
      <TableNodeContextMenu table={table}>
        ...
        >
          ...
          />
          <TableNodeDependencyIndicator table={table} focused={focused} />
          <TableNodeStatus
            status={
              isDiffNewTable
                ? 'new'
          ...
          <div className="group flex h-9 items-center justify-between bg-slate-200 px-2 dark:bg-slate-900">
            <div className="flex min-w-0 flex-1 items-center gap-2">
              ...
              {tableChangedName ? (
                <Label className="flex h-5 items-center justify-center truncate rounded-sm bg-sky-200 px-2 py-0.5 text-sm font-normal text-sky-900 dark:bg-sky-800 dark:text-sky-200">
                  ...
                  <span className="truncate">{tableChangedName}</span>
                </Label>
              ) : isDiffNewTable ? (
            ...
            <div className="hidden shrink-0 flex-row group-hover:flex">
              ...
              <Button
                ...
                className="size-6 p-0 text-slate-500 hover:bg-primary-foreground hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200"
                onClick={
                  table.width !== MAX_TABLE_SIZE ? expandTable : shrinkTable
                }
              >
