...
import { type DBField, generateDBFieldSuffix } from '@/lib/domain/db-field';
  ...
  ({ field, focused, tableNodeId, highlighted, visible, isConnectable }) => {
    ...
    const isTargetFromView = useMemo(
      () =>
        connection.inProgress &&
        connection.fromNode.id !== tableNodeId &&
        (connection.fromHandle.id?.startsWith(TOP_SOURCE_HANDLE_ID_PREFIX) ||
          connection.fromHandle.id?.startsWith(BOTTOM_SOURCE_HANDLE_ID_PREFIX)),
      [
        connection.inProgress,
        connection.fromNode?.id,
        connection.fromHandle?.id,
    ...
    return (
      <div
        className={cn(
          ...
          'transition-all duration-200 ease-in-out',
          {
            ...
            'bg-yellow-100 dark:bg-yellow-900': isCustomTypeHighlighted,
      ...
      >
        ...
        <div
          className={cn('flex items-center gap-1 min-w-0 flex-1 text-left', {
        ...
        >
          ...
          <span
            className={cn('truncate min-w-0', {
              'text-red-800 font-normal dark:text-red-200': isDiffFieldRemoved,
              'text-green-800 font-normal dark:text-green-200': isDiffNewField,
          ...
          >
            {fieldDiffChangedName ? (
              <>
                {field.name} <span className="font-medium">→</span>{' '}
                {fieldDiffChangedName}
              </>
            ) : (
              field.name
        ...
        <div className="ml-2 flex shrink-0 items-center justify-end gap-1.5">
          {(field.primaryKey && fieldDiffChangedPrimaryKey === null) ||
          fieldDiffChangedPrimaryKey ? (
            <div
              className={cn(
                'text-muted-foreground',
                !readonly ? 'group-hover:hidden' : '',
                isDiffFieldRemoved ? 'text-red-800 dark:text-red-200' : '',
                isDiffNewField ? 'text-green-800 dark:text-green-200' : '',
                isDiffFieldChanged &&
                  !isSummaryOnly &&
                  !isDiffFieldRemoved &&
                  !isDiffNewField
                ...
                `${field.type.name.split(' ')[0]}${
                  showFieldAttributes
                    ? generateDBFieldSuffix({
                        ...
                        ...{
                          precision:
                            fieldDiffChangedPrecision ?? field.precision,
                          scale: fieldDiffChangedScale ?? field.scale,
