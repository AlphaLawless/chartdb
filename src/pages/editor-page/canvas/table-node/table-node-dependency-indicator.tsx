...
export const TableNodeDependencyIndicator: React.FC<TableNodeDependencyIndicatorProps> =
  React.memo(({ table, focused }) => {
    ...
    const numberOfEdgesToTable = useMemo(
      () =>
        dependencies.filter((dependency) => dependency.tableId === table.id)
          .length,
    ...
    useEffect(() => {
      if (previousNumberOfEdgesToTableRef.current !== numberOfEdgesToTable) {
    ...
    return (
      <>
        ...
        {Array.from({ length: numberOfEdgesToTable }, (_, index) => index).map(
          (index) => (
            <Handle
