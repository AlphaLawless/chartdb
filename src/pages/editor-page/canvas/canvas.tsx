...
export const Canvas: React.FC<CanvasProps> = ({ initialTables }) => {
  ...
  useEffect(() => {
    const targetIndexes: Record<string, number> = relationships.reduce(
      (acc, relationship) => {
        acc[`${relationship.targetTableId}${relationship.targetFieldId}`] = 0;
        return acc;
      },
        ...
        ...tables.map((table) => {
          ...
          if (highlightedCustomType) {
            hasHighlightedCustomType = table.fields.some(
              (field) => field.type.name === highlightedCustomType.name
            );
          }
        ...
        updateTablesState(
          (currentTables) =>
            currentTables.map((table) => {
              const update = needsUpdate.find((u) => u.id === table.id);
              if (update) {
                return {
                  id: table.id,
  ...
  const onEdgesChangeHandler: OnEdgesChange<EdgeType> = useCallback(
    ...
    [getEdge, onEdgesChange, removeRelationships, removeDependencies, readonly]
  );
  ...
  const findRelevantNodesChanges = useCallback(
    (changes: NodeChange<NodeType>[], type: NodeType['type']) => {
      ...
      const positionChanges: NodePositionChange[] = relevantChanges.filter(
        (change) =>
          change.type === 'position' &&
          !change.dragging &&
          change.position?.x !== undefined &&
    ...
    (changes) => {
      ...
      if (areaDragChanges.length > 0) {
        ...
        areaDragChanges.forEach((areaChange) => {
          const currentArea = areas.find((a) => a.id === areaChange.id);
      ...
      > = new Map();
      if (areaPositionChanges.length > 0 && areaSizeChanges.length === 0) {
        areaPositionChanges.forEach((change) => {
          if (change.type === 'position' && change.position) {
            const currentArea = areas.find((a) => a.id === change.id);
            if (currentArea) {
              const deltaX = change.position.x - currentArea.x;
              const deltaY = change.position.y - currentArea.y;

              const childTables = getTablesInArea(change.id, tables);
              childTables.forEach((table) => {
              ...
              if (positionChange || sizeChange || areaMovement) {
                ...
                return {
                  ...
                  ...(areaMovement && !positionChange
                    ? {
                        x: currentTable.x + areaMovement.deltaX,
                        y: currentTable.y + areaMovement.deltaY,
                      }
                    : {}),
                  ...(sizeChange
                    ? {
                        width:
                          sizeChange.dimensions?.width ?? currentTable.width,
                      }
                    : {}),
                };
              }
  ...
  const eventConsumer = useCallback(
    (event: ChartDBEvent) => {
      ...
      if (event.action === 'add_tables') {
        ...
        setOverlapGraph(newOverlappingGraph);
      } else if (event.action === 'update_table' && event.data.table.width) {
  ...
  const hasOverlappingTables = useMemo(
    () => Array.from(overlapGraph.graph).some(([, value]) => value.length > 0),
    [overlapGraph]
  );
  ...
  return (
    <CanvasContextMenu>
      <div className="relative flex h-full" id="canvas" ref={containerRef}>
        <ReactFlow
        ...
        >
                ...
                <>
                      ...
                      <span>
                        <Button
                          ...
                          className={cn(
                            ...
                            snapToGridEnabled || shiftPressed
                  ...
                  {highlightedCustomType ? (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span>
                          <Button
                            ...
                            onClick={() => highlightCustomTypeId(undefined)}
                          >
                      ...
                      <TooltipContent>
                        {t('toolbar.custom_type_highlight_tooltip', {
                          typeName: highlightedCustomType.name,
                        })}
                      </TooltipContent>
                    </Tooltip>
                  ) : null}
          ...
          {isLostInCanvas ? (
            <Controls
              ...
              className="!shadow-none"
              style={{
                [isDesktop ? 'bottom' : 'top']: isDesktop ? '70px' : '70px',
              }}
            >
              <ShowAllButton />
            </Controls>
