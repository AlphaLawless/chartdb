  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (active?.id !== over?.id && !!over && !!active) {
        const oldIndex = fields.findIndex((field) => field.field === active.id);
        const newIndex = fields.findIndex((field) => field.field === over.id);
    ...
    <div className="flex flex-col gap-2 text-xs">
      <div className="flex flex-row items-center gap-1">
        ...
        <div className="font-bold text-subtitle">
          {t('side_panel.custom_types_section.custom_type.composite_fields')}
        </div>
      </div>
