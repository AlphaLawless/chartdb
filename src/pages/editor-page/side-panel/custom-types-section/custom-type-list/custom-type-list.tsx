...
export const CustomTypeList: React.FC<CustomTypeProps> = ({ customTypes }) => {
  ...
  const { openCustomTypeFromSidebar, openedCustomTypeInSidebar } = useLayout();
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {customTypes
            .sort((customType1: DBCustomType, customType2: DBCustomType) => {
              // if one has order and the other doesn't, the one with order should come first
              if (customType1.order && customType2.order === undefined) {
                return -1;
              }

              if (customType1.order === undefined && customType2.order) {
                return 1;
              }

              // if both have order, sort by order
              if (customType1.order != null && customType2.order != null) {
                return customType1.order - customType2.order;
              }

              // sort by name
              return customType1.name.localeCompare(customType2.name);
            })
            .map((customType) => (
              <CustomTypeListItem
                key={customType.id}
