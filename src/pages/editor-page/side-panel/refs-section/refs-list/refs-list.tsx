...
export const RefsList: React.FC<RefsListProps> = ({ refs }) => {
  ...
  const handleScrollToRef = useCallback(() => {
    if (openedRefInSidebar && lastOpenedRef.current !== openedRefInSidebar) {
  ...
  const relationshipsTitle = React.useMemo(
    () =>
      `${numberOfRelationships} ${t('side_panel.refs_section.relationships')}`,
  ...
  const dependenciesTitle = React.useMemo(
    () =>
      `${numberOfDependencies} ${t('side_panel.refs_section.dependencies')}`,
