...
export const AreaList: React.FC<AreaListProps> = ({ areas }) => {
  ...
  return (
    <div className="flex w-full flex-col gap-1">
      ...
      >
        <SortableContext items={areas} strategy={verticalListSortingStrategy}>
          {areas
            .sort((area1: Area, area2: Area) => {
              ...
              if (area1.order !== undefined && area2.order !== undefined) {
            ...
            .map((area) => (
              <AreaListItem key={area.id} area={area} ref={refs[area.id]} />
            ))}
        </SortableContext>
      </DndContext>
    </div>
