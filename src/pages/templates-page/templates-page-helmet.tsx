...
export const TemplatesPageHelmet: React.FC<TemplatesPageHelmetProps> = ({
  ...
  return (
    <Helmet>
      {needCanonical ? <link rel="canonical" href={canonicalUrl} /> : null}

      {tag ? (
        <title>{`${tag} database schema diagram templates | ChartDB`}</title>
      ) : isFeatured ? (
        <title>Featured database schema diagram templates | ChartDB</title>
      ) : (
      ...
      {tag ? (
        <meta property="og:url" content={`${HOST_URL}/templates/${tagParam}`} />
      ) : isFeatured ? (
        <meta property="og:url" content={`${HOST_URL}/templates/featured`} />
      ) : (
