...
>({
  ...
  useEffect(() => {
    if (selectable?.enabled && selectable.defaultSelectedId) {
      ...
      const { node, path } = findNodeById(data, selectable.defaultSelectedId);
      ...
      if (node) {
        ...
        for (const parent of path) {
          ...
          toggleNode(parent.id, parent.type, parent.context, parent.children);
        }
      }
    }
    ...
    <div className={cn(className)}>
      <div
        className={cn(
          ...
          isSelected
            ? 'bg-sky-500 border-sky-600 border dark:bg-sky-600 dark:border-sky-700'
        ...
        onDoubleClick={(e) => {
          e.stopPropagation();
          if (node.isFolder) {
            onToggle(node.id, node.type, node.context, node.children);
          }
        }}
      >
        <div className="flex flex-none items-center gap-1.5">
          ...
          ) : node.empty ? null : loadingNodeIds?.includes(node.id) ? (
            <Loader2
              className={cn('size-3.5 animate-spin', {
                // 'text-white': isSelected,
              })}
        ...
        {isHovered && renderHoverComponent ? renderHoverComponent(node) : null}
      </div>
        ...
        {isExpanded && children && (
          <motion.div
            ...
            animate={{
              ...
              transition: {
                height: {
                  duration: Math.min(0.3 + children.length * 0.018, 0.7),
                  ease: 'easeInOut',
                },
                opacity: {
                  duration: Math.min(0.2 + children.length * 0.012, 0.4),
                  ease: 'easeInOut',
                },
              },
            }}
            exit={{
              ...
              transition: {
                height: {
                  duration: Math.min(0.2 + children.length * 0.01, 0.45),
                  ease: 'easeInOut',
                },
                opacity: {
                  duration: 0.1,
