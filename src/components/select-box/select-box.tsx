...
export const SelectBox = React.forwardRef<HTMLInputElement, SelectBoxProps>(
  (
    ...
    const hasGroups = React.useMemo(
      () =>
        Object.keys(groups).filter((group) => group !== 'default').length > 0,
      [groups]
    );
          ...
          >
            ...
            <div className="flex flex-1 items-center truncate">
              ...
              <span>
                ...
                {!isRegexMatch && optionSuffix ? optionSuffix(option) : ''}
              </span>
              {option.description && (
            ...
            {((!multiple && option.value === value) || isRegexMatch) && (
              <CheckIcon
                className={cn(
                  'ml-auto',
        ...
        <PopoverTrigger asChild tabIndex={0} onKeyDown={handleKeyDown}>
          ...
          >
            ...
            <div className="flex items-center self-stretch pl-1 text-muted-foreground/60 hover:text-foreground [&>div]:flex [&>div]:items-center [&>div]:self-stretch">
              {value && value.length > 0 && multiple && showClear ? (
                <div
                  onClick={(e) => {
                    e.preventDefault();
                    handleClear();
        ...
        >
          <Command
            filter={(value, search, keywords) => {
              if (
                keywords?.length &&
                keywords.some((keyword) => new RegExp(keyword).test(search))
              ) {
                return 1;
              }
